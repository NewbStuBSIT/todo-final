...
public function signin($uname,$pasword)
    ...
    public function deleteTask($taskId) {
        $result = mysqli_query($this->dbh,"DELETE FROM tasks WHERE id='$taskId'");
        return $result;
    }

    public function deleteUser($userId) {
        $result = mysqli_query($this->dbh,"DELETE FROM tblusers WHERE id='$userId'");
        if (!$result) {
            error_log("Error deleting user: " . mysqli_error($this->dbh));
        }
        return $result;
    }
